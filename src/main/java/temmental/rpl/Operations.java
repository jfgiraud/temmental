    ...
    public static Number div(Number a, Number b) {
        ...
        if (a instanceof Long || b instanceof Long) {
            if (a.longValue() % b.longValue() == 0)
                ...
                return 1.0D * a.longValue() / b.longValue();
        }
        if (a instanceof Integer || b instanceof Integer) {
            if (a.intValue() / b.intValue() == 0)
                return a.intValue() / b.intValue();
            else
                return 1.0 * a.intValue() / b.intValue();
        }
        if (a instanceof Short || b instanceof Short) {
            if (a.shortValue() / b.shortValue() == 0)
                return a.shortValue() / b.shortValue();
            else
                return 1.0 * a.shortValue() / b.shortValue();
        }
        if (a instanceof Byte || b instanceof Byte) {
            if (a.byteValue() / b.byteValue() == 0)
                return a.byteValue() / b.byteValue();
            else
                return 1.0 * a.byteValue() / b.byteValue();
        }
        throw new RuntimeException("Add case not implemented.");
    }
